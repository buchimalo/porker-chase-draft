...
(function (global) {
    ...
    const FINISHERS = [
        ...
        {
            label: 'たった1枚',
            tagline: 'スペードのエース1枚。それで充分らしい',
            build: () => [card(14, 's')]
        },
        {
            label: 'フォーカード',
            tagline: 'まっとうに強い。ぐうの音も出ない',
            build: () => quads(anyRank()).concat([card(anyRank(), 'h')])
        },
        {
            label: 'エース全部＋ジョーカー',
            tagline: 'エースを4枚集めたうえに、まだ何か持っている',
    ...
    // 待ったをかけるチーム以外へは普通に5枚ずつ配る
    function dealObjection(teams, winner) {
        const deck = shuffle(newDeck());
        const finisher = pickFinisher();
        let cursor = 0;
        ...
        return teams.map(team => {
            if (team.id === winner.id) {
                return {
                    ...
                    finisher: finisher,
                    result: { category: 99, name: finisher.label, tiebreak: [] }
                };
            }
            const cards = deck.slice(cursor, cursor + 5);
            cursor += 5;
            return { team: team, cards: cards, result: evaluate(cards) };
        });
    }
    ...
    function deal(teams, options) {
        ...
        // 「ちょっとまったー！」— 勝者が無茶苦茶な札で全部蹴散らすパターン
        if (Math.random() < objectionChance) {
            const hands = dealObjection(teams, winner);
            const others = hands.filter(h => h.team.id !== winner.id)
                .sort((x, y) => compare(x.result, y.result));
            return {
                hands: hands,
                winner: winner,
                objection: true,
                provisional: others.length ? others[0].team : null,
