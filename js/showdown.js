...
(function (global) {
    ...
    const STRENGTH_TABLE = [
        { cat: 1, weight: 18 },   // ワンペア
        { cat: 2, weight: 30 },   // ツーペア
        { cat: 3, weight: 22 },   // スリーカード
        { cat: 4, weight: 12 },   // ストレート
    ...
    // 勝者に指定の役を持たせ、残りを配る。勝者が本当に最強かも確認する
    function dealStructured(teams, winner, category) {
        ...
        let cursor = 0;
        const hands = teams.map(team => {
            if (team.id === winner.id) {
                return { team: team, cards: winnerCards, result: evaluate(winnerCards) };
            }
        ...
        const wr = hands.filter(h => h.team.id === winner.id)[0].result;

        // 勝者が負けている / 同着 の配牌は使わない
        for (const o of others) {
            if (compare(o.result, wr) <= 0) return null;
        }
        // 相手が全員ハイカードだと寂しいので配り直す
        if (others.length && others.every(o => o.result.category === 0)) return null;
