...
(function () {
    ...
    async function runShowdown() {
        ...
        // --- 1〜4枚目 ---
        // 待ったをかけるチームは最後まで伏せたままにする
        const openIdx = packet.hands.map((h, i) => i)
            .filter(i => !(packet.objection && packet.hands[i].team.id === packet.winner.id));
        ...
        const order = openIdx
            ...
            .sort((a, b) => (a.hand.team.id === packet.winner.id ? 1 : 0) - (b.hand.team.id === packet.winner.id ? 1 : 0));
        ...
        if (packet.objection) {
            ...
            holder.innerHTML = winHand.cards.map(() => '<span class="pcard is-back"></span>').join('');
            await wait(320);

            for (let i = 0; i < winHand.cards.length; i++) {
                holder.querySelectorAll('.pcard')[i].outerHTML = Showdown.cardHtml(winHand.cards[i]);
                Showdown.sfx.slam(i);
                await wait(250);
            }
