...
(function () {
    ...
    async function runShowdown() {
        ...
        banner.textContent = '👑 ' + packet.comeback.leaderNames.join(' / ') + ' がリード';
    ...
    // 途中経過の役とリード表示を更新する
    function updateStandings(rows, packet, shown) {
        const leaderIds = Showdown.leadersAt(packet.hands, shown).map(t => t.id);
